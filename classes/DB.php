<?php
	...
	abstract class DB
	{
        ...
        $finalActor->setId($actor['actor_id']);
				...
				$finalActor->setFavorite_movie($actor['favorite_movie']);
		...
		public static function saveActor(Actor $actor)
		{
			...
			try {
				$stmt = $connection->prepare("
					INSERT INTO actors (first_name, last_name, rating, favorite_movie)
					VALUES(:id, :first_name, :last_name, :rating, :favorite_movie)
				");
				...
				$stmt->bindValue(':length', $actor->getFavorite_movie());
